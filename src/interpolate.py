...
import tempfile
from util import load_image, save_video_segment, create_video, concatenate_videos
...
class FrameInterpolator:
    ...
    def setup_model(self, gpu: bool, half: bool) -> torch.jit.ScriptModule:
        ...
        try:
            model = torch.jit.load("trained_model/film_net_fp16.pt", map_location=device)
    ...
    def load_and_prepare_images(self) -> tuple:
        ...
        img_batch_1, crop_region = load_image("output_frames/start_frame.png")
        img_batch_2, _ = load_image("output_frames/most_similar_frame.png")
    ...
    def generate_interpolated_frames(self, img_batch_1: torch.Tensor, img_batch_2: torch.Tensor, inter_frames: int) -> list:
    ...
    def process_frames(self, results: list, crop_region: tuple) -> list:
        ...
        frames = [(frame[0] * 255).byte().flip(0).permute(1, 2, 0).numpy()[y1:y2, x1:x2].copy() for frame in results]
        return frames

    def inference(self, video_path: str, interpolation_interval: float, most_similar_time: float, input_timestamps: list, target_timestamps: list, save_path: str, fps: int) -> None:
        ...
        results = self.generate_interpolated_frames(img_batch_1, img_batch_2, inter_frames)
        frames = self.process_frames(results, crop_region)

        logging.info(f'Generating final video.')
        with tempfile.NamedTemporaryFile(delete=True, suffix='.mp4') as temp_pre_interpolation:
            pre_interpolation_segment_path = temp_pre_interpolation.name
            save_video_segment(video_path, input_timestamps[0] - target_timestamps[0], input_timestamps[0], pre_interpolation_segment_path, fps)
            
            with tempfile.NamedTemporaryFile(delete=True, suffix='.mp4') as temp_interpolation_segment:
                interpolation_segment_path = temp_interpolation_segment.name
                create_video(frames, interpolation_segment_path, fps)
                
                with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as temp_post_interpolation:
                    post_interpolation_segment_path = temp_post_interpolation.name
                    save_video_segment(video_path, most_similar_time, None, post_interpolation_segment_path, fps)
